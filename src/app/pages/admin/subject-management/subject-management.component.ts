...
export class SubjectManagementComponent {
  ...
  ngOnInit() {
    this.getSession();
    this.http
      .get<any>('http://127.0.0.1:8000/api/subjects')
      .subscribe((data) => {
  ...
  subjectChange(value) {
    return (this.error.subject_name = null);
  }
  ...
  submitSubject() {
    ...
    if (this.form.subject_name == null || this.form.subject_name == '')
      this.error.subject_name = 'Semester is required.';

    if (
      this.form.semester_id == null ||
      this.form.semester_id == '' ||
      this.form.subject_name == null ||
    ...
    return this.http
      .post<any>('http://127.0.0.1:8000/api/subjects', this.form, { headers })
      .subscribe((data) => {
        if (data?.message) {
          this.reloadPage();
        }
          ...
          render: function (data, type, row) {
            return `<div class="d-flex gap-2">
                      <div>
                        ...
                        role="button" href="#myModal${row.subject_id}" 
                        data-bs-toggle="modal">Update</a>
                        <div
                          id="myModal${row.subject_id}"
                          class="modal fade"
                          role="dialog"
                          tabindex="-1"
                        >
                          <div class="modal-dialog modal-dialog-centered" role="document">
                            ...
                            <div class="modal-content">
                              <form method="POST" action="http://127.0.0.1:8000/api/subjects/update" target="dummyframe">
                                ...
                                <div class="modal-body">
                                  <div class="p-5">
                                    <input class="form-control" type="hidden" name="subject_id" value="${row.subject_id}"/>
                                    <div class="mb-3">
                                      <label for="email">Subject Name</label>
                                      <input class="form-control" type="text" name="subject_name" value="${row.subject_name}"/>
                                    </div>
                                  </div>
                                </div>
                      ...
                      <form method="POST" action="http://127.0.0.1:8000/api/subjects/delete" target="dummyframe">
                        <input class="form-control" type="hidden" name="subject_id" value="${row.subject_id}"/>
